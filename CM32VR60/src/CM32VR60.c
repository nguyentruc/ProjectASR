...
// Source: http://web.cse.ohio-state.edu/~cmcurtin/dvd/dvd_play/src/si/crc32.c
...
uint8_t sendData[40000];

extern void print(const char *format, ...);

uint32_t crc32 (char *data, uint16_t len);

void longToArr(uint32_t num, uint8_t *arr)
{
...
uint16_t makeSendData(uint16_t modelSize, CM32VR60_command cmd)
{
    uint16_t data_length = modelSize + 4;
    uint8_t numArray[4];

    print("\nShift CY_model 4 bytes right\n");
    memmove(sendData + 4, sendData, modelSize);

    longToArr(modelSize, numArray);

    print("\nCopy size to CY_model %u\n", modelSize);
    memcpy(sendData, numArray, 4);

    uint32_t crc = crc32((char *) sendData, data_length);
    longToArr(crc, numArray);

    print("\nCopy crc to CY_model\n");
    memcpy(sendData + data_length, numArray, 4);
    data_length += 4;

    memmove(sendData + 1, sendData, data_length);
    data_length += 1;
...
{
    memmove(sendData, CY_model, CY_model_length);

    uint16_t data_length = makeSendData(CY_model_length, cmd_initialization);

    print("\nOutput length: %u\n", data_length);

    HAL_I2C_Master_Transmit(CM32VR60_i2c_handle, IC_ADDR << 1, sendData,
            data_length, HAL_MAX_DELAY);
}

void CM32VR60_loadCommandModel(I2C_HandleTypeDef *CM32VR60_i2c_handle)
{
    //memmove(sendData, cmd_model, cmd_model_length);
    uint16_t data_length = makeSendData(cmd_model_length, cmd_load_command);
